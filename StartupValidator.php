        ...
        foreach ($grfFiles as $grfFile) {
            ...
            if (!$validation['valid']) {
                ...
                if (isset($validation['pathEncoding']['encoding']) && 
                    ...
                    // Check if path-mapping.json exists
                    $pathMappingFile = $CONFIGS['PATH_MAPPING_FILE'] ?? 'path-mapping.json';
                    $pathMappingExists = file_exists($pathMappingFile);
                    $pathMappingEnabled = $CONFIGS['PATH_MAPPING_ENABLED'] ?? false;
                    
                    if ($pathMappingExists && $pathMappingEnabled) {
                        // Path mapping exists and is enabled - show info instead of warning
                        $mappingSize = filesize($pathMappingFile);
                        $sizeFormatted = $mappingSize > 1048576 
                            ? number_format($mappingSize / 1048576, 2) . ' MB'
                            : number_format($mappingSize / 1024, 2) . ' KB';
                        $this->addInfo(
                            "Path mapping enabled: {$pathMappingFile} ({$sizeFormatted}) - Korean paths will be resolved"
                        );
                    } elseif ($pathMappingExists && !$pathMappingEnabled) {
                        // Path mapping exists but is disabled
                        $this->addWarning(
                            "GRF path encoding: {$grfFile} has non-UTF-8 filenames. " .
                            "path-mapping.json exists but PATH_MAPPING_ENABLED is false"
                        );
                    } else {
                        // No path mapping - show warning with samples
